...
class PingSafeAzureUnitAudit:
    ...
    def count_all(self):
        ...
        accounts, error = self.list_storage_accounts()
        if accounts is None:
            self.add_result("Storage Account", (0,error))
            self.add_result("Storage Blob Container", (0,error))
            self.add_result("Storage File Share", (0,error))
            self.add_result("Storage Queue", (0,error))
            self.add_result("Storage Table", (0,error))
        else:
            ...
            errors_blob_container = ""
            errors_file_share = ""
            errors_storage_queue = ""
            errors_storage_table = ""
            for account in accounts:
                key, error = self.get_storage_account_keys(account)
                if key is None:
                    errors = errors + error
                else:
                    results, error = self.count_storage_blob_container(account, key)
                    if results is None:
                        errors_blob_container = errors_blob_container + error
                    else:
                        count_storage_blob_container += results

                    results, error = self.count_storage_file_share(account, key)
                    if results is None:
                        errors_file_share = errors_file_share + error
                    else:
                        count_storage_file_share += results

                    results, error = self.count_storage_queue(account, key)
                    if results is None:
                        errors_storage_queue = errors_storage_queue + error
                    else:
                        count_storage_queue += results

                    results, error = self.count_storage_table(account, key)
                    if results is None:
                        errors_storage_table = errors_storage_table + error
                    else:
                        count_storage_table += results
            self.add_result("Storage Account", (self.count_storage_accounts(accounts), ""))
            self.add_result("Storage Blob Container", (count_storage_blob_container,errors_blob_container))
            self.add_result("Storage File Share", (count_storage_file_share,errors_file_share))
            self.add_result("Storage Queue", (count_storage_queue,errors_storage_queue))
            self.add_result("Storage Table", (count_storage_table,errors_storage_table))
